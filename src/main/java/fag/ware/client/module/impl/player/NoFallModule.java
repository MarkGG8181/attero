...
public class NoFallModule extends AbstractModule {
    private final StringSetting mode = new StringSetting("Mode", "Damage", "Damage", "Vanilla", "Verus");
    private final NumberSetting falldistance = new NumberSetting("Distance", 3.35, 1, 5);

    @Subscribe
    public void onTick(MotionEvent event) {
        if (event.isPre() && mc.player.fallDistance >= falldistance.toFloat()) {
